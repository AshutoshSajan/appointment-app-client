...
import { Row, Col, Button, Input, Space, Card, Layout, TimePicker, Alert } from 'antd'
...
function UserProfile(props) {
	...
	const fetchEvents = (url) => {
		fetch(`${url}/events`)
			...
			.then(({ events }) => {
				...
				const filteredHours = events.filter((event) => {
					...
					if (eventYear === currentYear && eventDate === currentDate && eventDay === currentDay) {
						return new Date(event.start).getHours()
					}
				})

				const unavailableHours = filteredHours.map((hour) => new Date(hour.start).getHours())
	...
	return (
		<Layout>
			{!error ? null : (
				<Alert style={{ position: 'absolute', width: '100%' }} message={error} type="error" />
			)}
